...
public class ToneTransformServiceImpl implements ToneTransformService {
    ...
    @Override
    public ToneTransformResponseDto transformTone(String originalText, Long toneId) {
        ...
        String transformedText = transformTextWithGPT(originalText, tone);

        // 응답 객체 생성
        return new ToneTransformResponseDto(transformedText, tone.getName());
    }

    private String transformTextWithGPT(String originalText, Tones tone) {
        ...
        payload.put("messages", List.of(
                Map.of(
                        ...
                        "content", "당신은 텍스트의 말투를 변환하는 AI 어시스턴트입니다. " +
                                "주어진 지침과 예시에 따라 원본 텍스트의 내용은 유지하면서 말투만 변경하세요. " +
                                "변환된 텍스트만 응답하고 다른 설명은 포함하지 마세요."
                ),
                Map.of(
                        "role", "user",
                        "content", "다음 텍스트를 '" + tone.getName() + "' 말투로 변환해주세요.\n\n" +
